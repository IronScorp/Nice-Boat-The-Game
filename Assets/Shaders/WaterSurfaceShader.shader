...
Properties {
	...
	_BumpScale ("Bump Scaling", Float) = 1.0
	...
	_FoamIntensity ("Foam intensity", Float) = 0.1
	_FoamCutoff ("Foam cutoff", Float) = 0.375
	_FoamScale ("Foam scaling", Float) = 1.0

	[Toggle(DRAW_WAVES)]
	_DrawWaves ("Draw waves", Float) = 1
}
...
CGINCLUDE
	...
	v2f vert(appdata_full v)
	{
		...
		if (_DrawWaves == 1){
			add_wave(worldSpaceVertex, v.normal, waves);
			add_noise(worldSpaceVertex, _BumpMap, waves);
		}

		v.vertex.y = worldSpaceVertex.y;
		...
		o.bumpCoords.xyzw = (tileableUv.xyxy + _Time.xxxx * _BumpDirection.xyzw) * _BumpTiling.xyzw * _BumpScale;
		...
		o.viewInterpolator.w = v.vertex.y * _FoamIntensity;
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		baseColor = lerp (lerp (rtRefractions, baseColor, baseColor.a), reflectionColor, refl2Refr * _ReflectionColor.a);
		baseColor = baseColor + spec * _SpecularColor;
		
		// handle foam
		half4 foam = Foam(_ShoreTex, i.bumpCoords * _FoamScale);
		baseColor.rgb += foam.rgb * _FoamIntensity * (edgeBlendFactors.y + saturate(i.viewInterpolator.w - _FoamCutoff));
